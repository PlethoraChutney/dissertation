...
While the pellets were thawing, 50 mL **Solubilization Buffer** (20 mM Tris pH 7.5, 200 mM KCl, 1 tablet per 100 mL Pierce EDTA-free protease inhibitor tablets, 25 U/mL ThermoFisher universal nuclease, 2.5 mM ATP, 5 mM MgCl~2~, 10 mg/mL digitonin, and 10 $\mu$M amiloride) was prepared per flask.
...
Meanwhile, in-house-made GFP-nanobody CNBr resin (0.8 mL per flask) was loaded into an XK 16 column and washed with 2 CV of **TBK** (20 mM Tris pH 7.5, 200 mM KCl) and the following buffers were prepared in the listed quantities:
**wash A** (10 column volumes [CV]): 20 mM Tris pH 7.5, 200 mM KCl, 0.7 mg/mL digitonin, 2 mM MgCl~2~, 2 mM ATP, 2 nM phenamil;
**wash B** (10 CV): 20 mM Tris pH 7.5, 200 mM KCl, 0.7 mg/mL digitonin, 5 mM CaCl~2~, 2 nM phenamil;
...
To elute ENaC from the GFP nanobody resin, the column was washed 3 times with 1.5 CV elution buffer and once with TBK + 0.7 mg/mL digitonin.
...
If the preparation was acid-shocked, the protein was desalted into **acid shock** buffer at this stage (20 mM Tris pH 6, 200 mM KCl, 0.7 mg/mL digitonin, 2 nM phenamil).
...
The concentrated eluant was loaded onto a Superose 6 Increase 10/300 column (Cytiva) equilibrated in **SEC buffer** (20 mM Tris pH 7.5, 200 mM KCl, 0.7 mg/mL digitonin, 2 nM phenamil) for further purification via SEC.
...
For each gram of membranes, 15&nbsp;mL of **solubilization buffer** (4&nbsp;mM GDN, 20&nbsp;mM HEPES pH&nbsp;7.6, 150&nbsp;mM NaCl, 2&nbsp;mM ATP, 2&nbsp;mM MgSO~4~, 1&nbsp;mM TCEP, 100&nbsp;&mu;M amiloride, 1&nbsp;tablet per 100&nbsp;mL Pierce EDTA-free protease inhibitor tablets) was added.
...
Next, 300&nbsp;&mu;L GFP nanobody CNBr resin per gram of membrane was loaded into a gravity column and washed with 10&nbsp;CV **HBS** (20&nbsp;mM HEPES pH&nbsp;7.6, 150&nbsp;mM NaCl) and 3&nbsp;CV **wash A** (0.1&nbsp;mg/mL GDN, 20&nbsp;mM HEPES pH 7.6, 150&nbsp;mM NaCl, 1&nbsp;mM TCEP, 100&nbsp;nM phenamil).
Clarified lysate was flowed over this column under gravity.
The column was washed with 6 CV each of wash A, **wash B** (wash A plus 2&nbsp;mM ATP, 2&nbsp;mM MgSO~4~), **wash C** (wash A plus 25 U/mL ThermoFisher Universal Nuclease), and **elution buffer** (wash A plus 5&nbsp;mM CaCl~2~).
ENaC was eluted by flowing 1&nbsp;CV of elution buffer plus 30&nbsp;&mu;g/mL thrombin onto the column and collecting the supernatant.
...
The lipids were then resuspended in 350 $\mu$L **reconstitution buffer** (20 mM HEPES pH 7.4, 100 mM KCl, 1 mM EDTA) and sonicated until the solution was milky in appearance and no lipids remained adhered to the sides of the tube, typically 5--10 minutes.
...
Grid preparation different slightly for each dataset, but followed the same general procedure (@tbl-model-building).
...
Most datasets were collected with at least one of our two pseudosymmetry-breaking Fabs, developed previously[@noreng18].
 I have switched from using both Fabs to only using one, because the &beta;-binding Fab is more flexible than the &alpha;-binding Fab, and this flexibility reduces overall alignment property.
Additionally, one of the two human CKO datasets was first incorporated into nanodiscs and solved without any Fabs.
As expected based on our previous studies of electrophysiology of the Fab-bound channels, our ECD models with and without Fab are largely indistinguishable (Supplemental @fig-fab-compare B,C)[@noreng20].
Neither fab significantly distorts the surrounding ECD environment when bound.
The &alpha;-binding Fab induces a slight shift in a region near the &beta;-ball, while the &beta;-binding Fab induces no detectable change at all.

## cryoEM Data Collection
Datasets were collected using SerialEM by microscopy core staff at the facilities used (@tbl-model-building)[@mastronarde2003].
...
From this point, processing proceeded *ad hoc* to produce the final maps (Supplemental Image Processing).
...
Second, scaling the CKO/DEG difab pixel size from the nominally-correct 0.43 to 0.414  (a change of approximately 3.6%) changes the map correlation value from 0.72 to 0.93 (Supplemental @fig-pixel-size-calibration A).
Finally, a comparison of the correlation between seven recently-collected ENaC maps shows that S^2^C^2^ is an outlier, with unscaled maps generated from data collected at PNCC and Janelia Farm correlating better with themselves and each other than with those collected at S^2^C^2^ regardless of construct (Supplemental @fig-pixel-size-calibration B).
...
Model quality was assessed using MolProbity within Phenix, Q-scores calculated in ChimeraX, and 3DFSC as implemented in cryoSPARC (@tbl-model-building and Supplemental Model Validation)[@williams2018; @pintilie2020; @aiyer2021].
...
#| error: false
#| results: asis
#| label: tbl-model-building
#| tbl-cap: Image processing and model building statistics.

library(tidyverse)
library(gt)
library(gtExtras)

particles <- read_csv('../figures/table-one/model-building.csv', na = '')
models <- read_csv('../figures/table-one/processed_tables.csv', col_types = 'cccccccc')
data <- bind_rows(particles, models)

# TODO: update Q-scores
data |> 
  add_row(
  ...
  tab_footnote(
    footnote = 'Resolutions here are nominal resolutions given by the microscopy facility. I have resion to believe the S\u00b2C\u00b2 pixel size is incorrect, and built the models into a corrected pixel size (see Materials and Methods).',
    locations = cells_stub(rows = 'Pixel Size (&Aring;/pix)')
  ) |>
  tab_footnote(
  ...
  tab_footnote(
    footnote = 'NAG, MAN, and BMA. UNK is counted as a residue instead.',
    locations = cells_stub(rows = 'No. ligands')
  ) |>
  tab_footnote(
    footnote = 'Residues modeled as UNK excluded from Q-score',
    locations = cells_stub(rows = 'Q-score')
  ) |>
  tab_options(
...
The four constructs I present here can broadly be split into those I expect to be open (CKO/DEG and mouse trypsin) and those I expect to be closed (CKO, mouse) (@tbl-model-building).
...
CKO channels express at approximately eight times the yield of wild-type-like channels, with similar electrophysiological traits (Supplemental @fig-ephys).
...
Cells were then pipetted over a dish of DMEM plus 10% FBS with several small coverslips and allowed to adhere at 37&nbsp;&deg;C while the electrophysiology rig was prepared, approximately 2 hours.
Pipettes were pulled to 2--3 M$\Omega$ resistance and filled with filtered **internal solution** (150 mM KCl, 2 mM MgCl~2~, 5 mM EGTA, 10 mM HEPES pH 7.4).
Working one coverslip at a time, cells were moved from DMEM at 37&nbsp;&deg;C to a dish on the rig (at room temperature) containing **K^+^ external solution** (150 mM KCl, 2 mM MgCl~2~, 2 mM CaCl~2~, and 10 mM HEPES pH 7.4).
...
Finally, a KCl + 5 $\mu$g/mL **trypsin solution** was prepared.
