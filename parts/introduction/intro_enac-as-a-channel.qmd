...
Finally, I provide a brief overview of proteins which directly modify ENaC's electrophysiological properties (rather than expression levels, which are covered in @sec-intro_enac-in-body).
...
As discussed further in @sec-intro_conduction-tmd, ENaC's selectivity filter is composed of a triplet residues in a domain-swapping loop between adjacent transmembrane helices.
...
As they do not have a ligand *per se*, ENaC channels do not desensitize in the traditional sense.
...
SSI reduces ENaC currents as a direct result of extracellular Na^+^ concentrations, not as a result of the currents themselves [@chraibi2002].
Lower temperatures reduces the magnitude of SSI, resulting in greater ENaC currents [@chraibi2002; @askwith2001].
...
It is reasonable to expect, then, that the Na^+^-binding site is composed at least in part by the GRIP domain.
...
Surprisingly, despite its effect on magnitude, pH does not have an effect on Na^+^ K~i~.
...
A reduction in affinity should thereby reduce Na^+^ K~i~.
...
However, a recent structure of ASIC revealed that the N-terminus briefly re-enters the membrane to buttress the GAS belt (@fig-intro_hg-motif) [@yoder20].
...
Although I did not succeed in solving a structure of the TMD, high-resolution structures of the channel in cleaved and uncleaved states inform us that ECD/TMD coupling is important for ECD conformational changes (@sec-chapter-tmd).
My work on the latter goal has revealed a number of new glycans which seem to form contacts among the subunits and with the membrane/micelle (@sec-results_ecd).
Additionally, the acidic pocket may not be involved in SSI, and the ECD does not undergo appreciable conformational change between Na^+^ and K^+^ buffers.